...
public class Vision extends SubsystemBase {
    ...
    public void periodic() {
        ...
        if (cameraToTargetTranslations.size() >= minTargetCount * 4) {
            finalTranslation2d = 
                CircleFitter.fit(VisionConstants.TARGET_DIAMETER / 2.0,
    ...
    public double desiredRobotAngle() {
        ...
        //double m1 = (getVisionPose().getY() - Constants.HUB_POSE.getY())/(getVisionPose().getX() - Constants.HUB_POSE.getX());
        double m2 = 0;
        double m3 = (getVisionPose().getY() - LLPose(1, gyro.getRotation2d().getRadians()).getY())/(getVisionPose().getX() - LLPose(1, gyro.getRotation2d().getRadians()).getX());

        //Line desired = new Line(m1, Constants.HUB_POSE.getX(), Constants.HUB_POSE.getY());
        ...
        double a = distanceTo(new double[]{getVisionPose().getX(), getVisionPose().getY()}, new double[]{Constants.HUB_POSE.getX(), Constants.HUB_POSE.getY()});
        double b = distanceTo(new double[]{getVisionPose().getX(), getVisionPose().getY()}, current.intersection(horizontal));
        double c = distanceTo(new double[]{Constants.HUB_POSE.getX(), Constants.HUB_POSE.getY()}, current.intersection(horizontal));

        double angle = Units.radiansToDegrees(Math.acos((Math.pow(a, 2) + Math.pow(b, 2) - Math.pow(c, 2) )/ (2 * a * b)));

        return angle;
    }

    public static double verifyDesiredRobotAngle(double[] hub, double[] p, double cAngle) {
     ...
     //   double m1 = (p[1] - hub[1])/(p[0] - hub[0]);
      ...
      //  Line desired = new Line(m1, hub[0], hub[1]);
        ...
        double a = distanceTo(new double[]{p[0], p[1]}, new double[]{hub[0], hub[1]});
        double b = distanceTo(new double[]{p[0], p[1]}, current.intersection(horizontal));
        double c = distanceTo(new double[]{hub[0], hub[1]}, current.intersection(horizontal));

        System.out.println(current.intersection(horizontal)[0] + ", " + current.intersection(horizontal)[1]);
    ...
    public static Translation2d tLLPose(double[] c, double d, double ang){
        return new Translation2d(c[0] + (d * Math.cos(ang)), c[1] + (d * Math.sin(ang)));
    }

    private Translation2d LLPose(double d, double dAngle){
        return new Translation2d(getVisionPose().getX() + (d * Math.cos(Units.degreesToRadians(dAngle))), getVisionPose().getY() + (d * Math.sin(Units.degreesToRadians(dAngle))));
    }
